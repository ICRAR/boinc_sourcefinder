        ...
        for run_id in self.run_ids:
            ...
            for cube in self.connection.execute(select([CUBE]).where(and_(CUBE.c.run_id == run_id, CUBE.c.progress == 2))):
                ...
                if existing_cube is None:
                    ...
                    print "Cube {0} already exists. ID: {1}. Progress: {2}.".format(cube["cube_name"], cube["cube_id"], cube["progress"])
    ...
    def _load_results(self, original_cube_id, cube):
        ...
        for result in self.connection.execute(select([RESULT]).where(RESULT.c.cube_id == original_cube_id)):
            ...
            try:
                ...
                if self.connection_result.execute(select([SOURCE]).where(SOURCE.c.original_id == result["result_id"])).fetchone() is None:
